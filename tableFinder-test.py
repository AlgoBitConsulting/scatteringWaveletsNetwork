...
def allColumnsOfTables(C2, rL, GEN, page, xmm, white=255):

   print("calculating column lines...")
   
   BL,WL             = GEN.groupingInLine(GEN.pathToPDF, GEN.pdfFilename, "/home/markus/anaconda3/python/pngs/challenge/test/", page)
   IMGL              = []
   MIDL              = []
   BOXL              = []
   ERG               = []
   ...
   for ii in range(len(rL)):
      ...
      if len(K) > 0:
         ...
         midL3                 = list(filter(lambda x: len( list(filter(lambda y: y[0] <= x <= y[2] , KxL_small))) <= 1, midL2))
...
#***
#
#  exec(open("tableFinder-test.py").read())
#
   ...
   for kindOfBox in INFO.KBOX:
      for method in INFO.MBOX:
         for direction in INFO.DBOX:
            ...
            DATAt = MISC.loadIt(INFO.path + fname + date)
            DATA  = boxMaster()
            DATA.INFO = DATAt.INFO
            DATA.rf   = DATAt.rf
            #MISC.saveIt(DATA,'/home/markus/anaconda3/python/development/' + fname + date)
...
challengeJPG.con     = challengeJPG.engine.connect()
...
for dd in 1*P:
   ...
   IMGL, MIDL, BOXL    = allColumnsOfTables(Corg.copy(), rLN_TA, generator, page, DATA.xmm)
